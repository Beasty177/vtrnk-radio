...
class AudioPlayerHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  ...
  AudioPlayerHandler() {
    ...
    _player.setLoopMode(LoopMode.off); // Явно отключаем loop для continuous stream
    _player.processingStateStream.listen((state) {
      if (state == ProcessingState.completed) {
        print("Stream completed unexpectedly - reconnecting");
        reloadStream(); // Auto-reconnect если стрим "закончился"
        _player.play();
      }
    });
    _loadInitialTrack();
  ...
  Future<void> _loadInitialTrack() async {
    await reloadStream(); // Используем reload для инициализации
  }
  ...
  Future<void> play() async {
    try {
      if (!_player.playing) {
        await reloadStream(); // Переподключение при play
      }
      await _player.play();
    } catch (e) {
