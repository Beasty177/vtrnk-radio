...
class _MyHomePageState extends State<MyHomePage> with SingleTickerProviderStateMixin {
  ...
  Color _backgroundColor = Colors.black; // Динамический фон
  ...
  void initState() {
    ...
    _fetchTrackInfo();
    _initWebSocket();
    _updateBackgroundColor(); // Инициализация цвета фона
  }
        ...
        (message) {
          ...
          if (data is Map && !isVideoStreamActive(data)) {
            setState(() {
              ...
              _fetchCoverUrl(); // Обновляем обложку при изменении трека
              _updateBackgroundColor(); // Обновляем цвет фона
            });
          }
        },
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _fetchCoverUrl(); // Обновляем обложку
          _updateBackgroundColor(); // Обновляем цвет фона
        });
      }
    } catch (e) {
  ...
  Future<void> _fetchCoverUrl() async {
    try {
      final response = await http.get(Uri.parse('https://vtrnk.online/get_cover_path'));
      if (response.statusCode == 200) {
        setState(() {
          _coverUrl = 'https://vtrnk.online${response.body}'; // Предполагаем путь от сервера
        });
      }
    } catch (e) {
  ...
  Future<void> _updateBackgroundColor() async {
    try {
      ...
      setState(() {
        _backgroundColor = palette.dominantColor?.color ?? Colors.black;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _backgroundColor, // Динамический фон
      appBar: AppBar(
