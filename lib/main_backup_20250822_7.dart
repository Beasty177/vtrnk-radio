...
class _MyHomePageState extends State<MyHomePage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _fetchTrackInfo(); // Начальная загрузка
    _initWebSocket(); // Подключение WebSocket
  }
    ...
    try {
      ...
      _channel.stream.listen(
        (message) {
          ...
          if (data is Map && !isVideoStreamActive(data)) { // Проверяем, если видео не активно
            setState(() {
  ...
  bool isVideoStreamActive(Map? data) {
    // Логика из HTML: проверяем статус видео-стрима (примерно)
