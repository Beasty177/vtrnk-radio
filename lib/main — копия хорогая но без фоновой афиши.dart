...
class _MyHomePageState extends State<MyHomePage> with TickerProviderStateMixin {
  ...
  late AnimationController _controller; // Для эквалайзера
  late AnimationController _colorController; // Для анимации цвета фона
  late Animation<Color?> _colorAnimation; // Анимация перехода цвета
  ...
  void initState() {
    ...
    // Инициализация анимации цвета фона
  ...
  void _updateMediaMetadata() {
    ...
    _audioHandler.updateMediaItem(newItem);
  }
  Future<void> _updateBackgroundColor() async {
    try {
      // Для перехода на flutter_colorpalette замените PaletteGenerator на:
      // final palette = await ColorPalette.fromImageProvider(NetworkImage(_coverUrl));
      // Color newColor = palette.dominantColor ?? Colors.black;
      ...
      if (mounted) {
        ...
        // Запускаем анимацию перехода цвета
        setState(() {
          _colorAnimation = ColorTween(
            begin: _backgroundColor,
            end: targetColor,
          ...
          _backgroundColor = targetColor; // Обновляем конечный цвет
        });
        _colorController.forward(from: 0.0); // Запускаем анимацию
      }
    } catch (e) {
...
class AudioPlayerHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  ...
  Future<void> _loadInitialTrack() async {
    try {
      ...
      print("Ошибка загрузки начального трека: $e");
    }
  }
