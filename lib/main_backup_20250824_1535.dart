...
class _MyHomePageState extends State<MyHomePage> with SingleTickerProviderStateMixin {
    ...
    _socket.on('track_update', (data) {
      ...
      if (data is Map && !isVideoStreamActive(data)) {
        setState(() {
          ...
          _fetchCoverUrl(); // Запрос обложки при обновлении трека
          _updateBackgroundColor();
        });
      }
    });
  ...
  Future<void> _fetchTrackInfo() async {
    try {
      ...
      await _fetchCoverUrl(); // Запрос обложки при инициализации
    } catch (e) {
  ...
  Future<void> _fetchCoverUrl() async {
    try {
      ...
      print("Ответ от /get_cover_path: ${coverResponse.body}"); // Отладка
      if (coverResponse.statusCode == 200) {
        ...
        setState(() {
          ...
          print("Обложка обновлена: $_coverUrl"); // Отладка
          _updateBackgroundColor();
        });
      } else {
